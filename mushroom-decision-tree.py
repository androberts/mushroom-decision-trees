...
def find_best_split(data):
    ...
    for i, col in enumerate(data.columns[1:]):
        ...
        for val in values:
            ...
            true_edible = len([row for row in true_rows if row[class_idx] == 'e'])
            true_poisonous = len([row for row in true_rows if row[class_idx] == 'p'])
            false_edible = len([row for row in false_rows if row[class_idx] == 'e'])
            false_poisonous = len([row for row in false_rows if row[class_idx] == 'p'])
    ...
    columns = ['Question', 'InfoGain', 'NumTrue', 'GiniTrue', 'EdibleTrue', 'PoisonousTrue', 'NumFalse', 'GiniFalse',
               'EdibleFalse', 'PoisonousFalse']

    return best_gain, best_question, pd.DataFrame(summary, columns=columns).sort_values('InfoGain', ascending=False)
...
print('Highest info gain:')
print(summary.head())
print('Lowest info gain:')
print(summary.tail())
